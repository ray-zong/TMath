...
TMATH_BEGIN_NAMESPACE
    ...
    const TSGP4::CommonConstants TSGP4::Empty_CommonConstants = TSGP4::CommonConstants();
const TSGP4::NearSpaceConstants TSGP4::Empty_NearSpaceConstants = TSGP4::NearSpaceConstants();
const TSGP4::DeepSpaceConstants TSGP4::Empty_DeepSpaceConstants = TSGP4::DeepSpaceConstants();
const TSGP4::IntegratorConstants TSGP4::Empty_IntegratorConstants = TSGP4::IntegratorConstants();
const TSGP4::IntegratorParams TSGP4::Empty_IntegratorParams = TSGP4::IntegratorParams();

TSGP4::TSGP4(const TTle &tle)
    :m_orbitalElements(tle)
{
    initialise();
}
...
void TSGP4::setTle( const TTle &tle )
{
    /*
    * extract and format tle data
    */
    m_orbitalElements = TOrbitalElements(tle);

    initialise();
}

TECI TSGP4::findPosition( double tsince ) const
{
    if (m_use_deep_space)
    {
...
TECI TSGP4::findPosition( const TDateTime &dateTime ) const
{
    return findPosition((dateTime - m_orbitalElements.epoch()).totalMinutes());
}

void TSGP4::initialise()
{
    /*
    ...
    * error checks
    */
    if (m_orbitalElements.eccentricity() < 0.0 || m_orbitalElements.eccentricity() > 0.999)
    {
        throw TSatelliteException("Eccentricity out of range");
    }

    if (m_orbitalElements.inclination() < 0.0 || m_orbitalElements.inclination() > kPI)
    {
        throw TSatelliteException("Inclination out of range");
    }

    common_consts_.cosio = cos(m_orbitalElements.inclination());
    common_consts_.sinio = sin(m_orbitalElements.inclination());
    const double theta2 = common_consts_.cosio * common_consts_.cosio;
    common_consts_.x3thm1 = 3.0 * theta2 - 1.0;
    const double eosq = m_orbitalElements.eccentricity() * m_orbitalElements.eccentricity();
    const double betao2 = 1.0 - eosq;
    const double betao = sqrt(betao2);

    if (m_orbitalElements.period() >= 225.0)
    {
    ...
    else
    {
        ...
        */
        if (m_orbitalElements.perigee() < 220.0)
        {
    ...
    */
    ...
    if (m_orbitalElements.perigee() < 156.0)
    {
        s4 = m_orbitalElements.perigee() - 78.0;
        if (m_orbitalElements.perigee() < 98.0) 
        {
            s4 = 20.0;
        }
    ...
    */
    const double pinvsq = 1.0
        / (m_orbitalElements.recoveredSemiMajorAxis()
        * m_orbitalElements.recoveredSemiMajorAxis()
        * betao2 * betao2);
    const double tsi = 1.0 / (m_orbitalElements.recoveredSemiMajorAxis() - s4);
    common_consts_.eta = m_orbitalElements.recoveredSemiMajorAxis()
        * m_orbitalElements.eccentricity() * tsi;
    const double etasq = common_consts_.eta * common_consts_.eta;
    const double eeta = m_orbitalElements.eccentricity() * common_consts_.eta;
    const double psisq = fabs(1.0 - etasq);
    const double coef = qoms24 * pow(tsi, 4.0);
    const double coef1 = coef / pow(psisq, 3.5);
    const double c2 = coef1 * m_orbitalElements.recoveredMeanMotion()
        * (m_orbitalElements.recoveredSemiMajorAxis()
        * (1.0 + 1.5 * etasq + eeta * (4.0 + etasq))
        + 0.75 * kCK2 * tsi / psisq * common_consts_.x3thm1
        * (8.0 + 3.0 * etasq * (8.0 + etasq)));
    common_consts_.c1 = m_orbitalElements.bStar() * c2;
    common_consts_.a3ovk2 = -kXJ3 / kCK2 * kAE * kAE * kAE;
    common_consts_.x1mth2 = 1.0 - theta2;
    common_consts_.c4 = 2.0 * m_orbitalElements.recoveredMeanMotion()
        * coef1 * m_orbitalElements.recoveredSemiMajorAxis() * betao2
        * (common_consts_.eta * (2.0 + 0.5 * etasq) + m_orbitalElements.eccentricity()
        * (0.5 + 2.0 * etasq)
        - 2.0 * kCK2 * tsi / (m_orbitalElements.recoveredSemiMajorAxis() * psisq)
        * (-3.0 * common_consts_.x3thm1 * (1.0 - 2.0 * eeta + etasq
        * (1.5 - 0.5 * eeta))
        + 0.75 * common_consts_.x1mth2 * (2.0 * etasq - eeta *
        (1.0 + etasq)) * cos(2.0 * m_orbitalElements.argumentPerigee())));
    const double theta4 = theta2 * theta2;
    const double temp1 = 3.0 * kCK2 * pinvsq * m_orbitalElements.recoveredMeanMotion();
    const double temp2 = temp1 * kCK2 * pinvsq;
    const double temp3 = 1.25 * kCK4 * pinvsq * pinvsq * m_orbitalElements.recoveredMeanMotion();
    common_consts_.xmdot = m_orbitalElements.recoveredMeanMotion() + 0.5 * temp1 * betao *
    ...
    if (m_use_deep_space)
    {
        deepspace_consts_.gsto = m_orbitalElements.epoch().toGreenwichSiderealTime();
    ...
    {
        double c3 = 0.0;
        if (m_orbitalElements.eccentricity() > 1.0e-4)
        {
            c3 = coef * tsi * common_consts_.a3ovk2 * m_orbitalElements.recoveredMeanMotion() * kAE *
                common_consts_.sinio / m_orbitalElements.eccentricity();
        }

        nearspace_consts_.c5 = 2.0 * coef1 * m_orbitalElements.recoveredSemiMajorAxis() * betao2 * (1.0 + 2.75 *
            (etasq + eeta) + eeta * etasq);
        nearspace_consts_.omgcof = m_orbitalElements.bStar() * c3 * cos(m_orbitalElements.argumentPerigee());

        nearspace_consts_.xmcof = 0.0;
        if (m_orbitalElements.eccentricity() > 1.0e-4)
        {
            nearspace_consts_.xmcof = -kTWOTHIRD * coef * m_orbitalElements.bStar() * kAE / eeta;
        }

        nearspace_consts_.delmo = pow(1.0 + common_consts_.eta * (cos(m_orbitalElements.meanAnomoly())), 3.0);
        nearspace_consts_.sinmo = sin(m_orbitalElements.meanAnomoly());

        if (!m_use_simple_model)
        {
            const double c1sq = common_consts_.c1 * common_consts_.c1;
            nearspace_consts_.d2 = 4.0 * m_orbitalElements.recoveredSemiMajorAxis() * tsi * c1sq;
            const double temp = nearspace_consts_.d2 * tsi * common_consts_.c1 / 3.0;
            nearspace_consts_.d3 = (17.0 * m_orbitalElements.recoveredSemiMajorAxis() + s4) * temp;
            nearspace_consts_.d4 = 0.5 * temp * m_orbitalElements.recoveredSemiMajorAxis() *
                tsi * (221.0 * m_orbitalElements.recoveredSemiMajorAxis() + 31.0 * s4) * common_consts_.c1;
...
TECI TSGP4::findPositionSDP4( const double tsince ) const
{
    /*
    * the final values
    */
    ...
    */
    double xmdf = m_orbitalElements.meanAnomoly()
        + common_consts_.xmdot * tsince;
    double omgadf = m_orbitalElements.argumentPerigee()
        + common_consts_.omgdot * tsince;
    const double xnoddf = m_orbitalElements.ascendingNode()
    ...
    double tempe = m_orbitalElements.bStar() * common_consts_.c4 * tsince;
    double templ = common_consts_.t2cof * tsq;

    double xn = m_orbitalElements.recoveredMeanMotion();
    e = m_orbitalElements.eccentricity();
    xincl = m_orbitalElements.inclination();

    deepSpaceSecular(tsince, xmdf, omgadf, xnode, e, xincl, xn);

    if (xn <= 0.0)
    {
        throw TSatelliteException("Error: (xn <= 0.0)");
    }

    a = pow(kXKE / xn, kTWOTHIRD) * tempa * tempa;
    e -= tempe;
    double xmam = xmdf + m_orbitalElements.recoveredMeanMotion() * templ;
    ...
    */
    if (e <= -0.001)
    {
        throw TSatelliteException("Error: (e <= -0.001)");
    }
    else if (e < 1.0e-6)
    {
        e = 1.0e-6;
...
TECI TSGP4::findPositionSGP4( double tsince ) const
{
    /*
    * the final values
    */
    ...
    * update for secular gravity and atmospheric drag
    */
    const double xmdf = m_orbitalElements.meanAnomoly()
        + common_consts_.xmdot * tsince;
    const double omgadf = m_orbitalElements.argumentPerigee()
        + common_consts_.omgdot * tsince;
    const double xnoddf = m_orbitalElements.ascendingNode()
        + common_consts_.xnodot * tsince;
    ...
    if (!m_use_simple_model)
    {
        ...
        tempe += m_orbitalElements.bStar() * nearspace_consts_.c5
    ...
    a = m_orbitalElements.recoveredSemiMajorAxis() * tempa * tempa;
    e = m_orbitalElements.eccentricity() - tempe;
    xl = xmp + omega + xnode + m_orbitalElements.recoveredMeanMotion() * templ;
...
TECI TSGP4::calculateFinalPositionVelocity( const double tsince, const double e, const double a, 
    ...
    const double cosio, const double sinio ) const
{
    ...
    */
    ...
    if (elsq >= 1.0)
    {
        throw TSatelliteException("Error: (elsq >= 1.0)");
    }
    ...
    */
    ...
    if (pl < 0.0)
    {
        throw TSatelliteException("Error: (pl < 0.0)");
    }
    ...
    */
    ...
    TVector<double, 3> position(x, y, z);
    ...
    TVector<double, 3> velocity(xdot, ydot, zdot);

    if (rk < 1.0)
    {
        //TODO
        //throw DecayedException(
        //    m_orbitalElements.epoch().addMinutes(tsince),
        //    position,
        //    velocity);
    }

    return TECI(m_orbitalElements.epoch().addMinutes(tsince), position, velocity);
}
...
void TSGP4::deepSpaceInitialise( const double eosq, const double sinio, const double cosio, 
    ...
    const double xmdot, const double omgdot, const double xnodot )
{
    ...
    const double aqnv = 1.0 / m_orbitalElements.recoveredSemiMajorAxis();
    const double xpidot = omgdot + xnodot;
    const double sinq = sin(m_orbitalElements.ascendingNode());
    const double cosq = cos(m_orbitalElements.ascendingNode());
    const double sing = sin(m_orbitalElements.argumentPerigee());
    const double cosg = cos(m_orbitalElements.argumentPerigee());

    /*
    * initialize lunar / solar terms
    */
    const double jday = m_orbitalElements.epoch().toJulian() - kEPOCH_JAN1_12H_2000;
    ...
    deepspace_consts_.zmol = WrapTwoPI(c - gam);
    ...
    deepspace_consts_.zmos = WrapTwoPI(6.2565837 + 0.017201977 * jday);
    ...
    */
    ...
    double zn = ZNS;
    double ze = ZES;
    const double xnoi = 1.0 / m_orbitalElements.recoveredMeanMotion();
    ...
    for (int cnt = 0; cnt < 2; cnt++)
    {
        ...
        */
        ...
        const double s1 = -15.0 * m_orbitalElements.eccentricity() * s4;
        ...
        */
        if (m_orbitalElements.inclination() < 5.2359877e-2
            || m_orbitalElements.inclination() > kPI - 5.2359877e-2)
        {
            shdq = 0.0;
        }
        else
    ...
    if (m_orbitalElements.recoveredMeanMotion() < 0.0052359877
        && m_orbitalElements.recoveredMeanMotion() > 0.0034906585)
    {
        /*
        ...
        */
        ...
        deepspace_consts_.del1 = 3.0 * m_orbitalElements.recoveredMeanMotion()
            * m_orbitalElements.recoveredMeanMotion()
            * aqnv * aqnv;
        ...
        integrator_consts_.xlamo = m_orbitalElements.meanAnomoly()
            + m_orbitalElements.ascendingNode()
            + m_orbitalElements.argumentPerigee()
    ...
    else if (m_orbitalElements.recoveredMeanMotion() < 8.26e-3
        || m_orbitalElements.recoveredMeanMotion() > 9.24e-3
        || m_orbitalElements.eccentricity() < 0.5)
    {
    ...
    else
    {
        ...
        */
        ...
        double g201 = -0.306 - (m_orbitalElements.eccentricity() - 0.64) * 0.440;

        if (m_orbitalElements.eccentricity() <= 0.65)
        {
            g211 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                3.616, -13.247, +16.290, 0.0);
            g310 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                -19.302, 117.390, -228.419, 156.591);
            g322 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                -18.9068, 109.7927, -214.6334, 146.5816);
            g410 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                -41.122, 242.694, -471.094, 313.953);
            g422 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                -146.407, 841.880, -1629.014, 1083.435);
            g520 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
        ...
        else
        {
            ...
            if (m_orbitalElements.eccentricity() <= 0.715)
            {
                g520 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
        ...
        if (m_orbitalElements.eccentricity() < 0.7)
        {
            g533 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                -919.2277, 4988.61, -9064.77, 5542.21);
            g521 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
                -822.71072, 4568.6173, -8491.4146, 5337.524);
            g532 = evaluateCubicPolynomial(m_orbitalElements.eccentricity(),
        ...
        const double xno2 = m_orbitalElements.recoveredMeanMotion()
            * m_orbitalElements.recoveredMeanMotion();
    ...
    if (initialise_integrator)
    {
        ...
        */
        integrator_consts_.xfact = bfact - m_orbitalElements.recoveredMeanMotion();
        integrator_params_.atime = 0.0;
        integrator_params_.xni = m_orbitalElements.recoveredMeanMotion();
...
void TSGP4::deepSpaceCalculateLunarSolarTerms( const double tsince, double& pe, double& pinc, 
...
void TSGP4::deepSpacePeriodics( const double tsince, double& em, double& xinc, 
...
void TSGP4::deepSpaceSecular( const double tsince, double& xll, double& omgasm, 
...
void TSGP4::deepSpaceCalcDotTerms( struct IntegratorValues& values ) const
{
    ...
    {
        const double xomi = m_orbitalElements.argumentPerigee()
...
void TSGP4::deepSpaceIntegrator( const double delt, const double step2, const struct IntegratorValues& values ) const
{
    /*
    * integrator
    */
